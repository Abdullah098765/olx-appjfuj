...
export default function Edit_Form (params) {
  ...
  var [adData, setAdData] = useState(edit_ad)
  const [check, setCheck] = useState(false)

  let { setAdsList } = useContext(AppContext)

  useEffect(() => {
    let city = document.getElementsByTagName('option')
    for (let i = 0; i < city.length; i++) {
      const element_value = city[i].value
      if (adData.city === element_value) {
        console.log(element_value)
        city[i].setAttribute('selected', 'true')
      }
    }
...
console.log('w0rking');
  ...
  function submit (params) {
    ...
    fetch('http://localhost:5001/edit_ad', requestOptions)
